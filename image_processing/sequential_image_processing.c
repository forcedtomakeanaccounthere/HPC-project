...
// Original image processing functions
...
int main(int argc, char* argv[]) {
    ...
    #else
        system("mkdir -p \"sequential output images\"");
    #endif
    ...
    snprintf(output_filename, sizeof(output_filename), "sequential output images/%s_grayscale.png", output_prefix);
    ...
    snprintf(output_filename, sizeof(output_filename), "sequential output images/%s_blur.png", output_prefix);
    ...
    snprintf(output_filename, sizeof(output_filename), "sequential output images/%s_sharp.png", output_prefix);
    ...
    snprintf(output_filename, sizeof(output_filename), "sequential output images/%s_noise.png", output_prefix);
    ...
    start_time = get_time();
    compress_image_multilevel(compress_img, output_prefix, 3); // 3 compression levels only
    end_time = get_time();
...
// NEW FUNCTIONS FOR MULTI-LEVEL COMPRESSION

void apply_gaussian_prefilter(Image* img, float sigma) {
    // HOTSPOT 6: Pre-filtering before downsampling to prevent aliasing
    // This is essentially the same as Gaussian blur but with different purpose
    apply_gaussian_blur(img, sigma);
}

Image* downsample_image(Image* img, int scale_factor) {
    // HOTSPOT 7: Downsampling operation - memory intensive
    ...
    for (int y = 0; y < new_height; y++) {
        for (int x = 0; x < new_width; x++) {
            for (int c = 0; c < img->channels; c++) {
                ...
                // Average pixels in the scale_factor x scale_factor block
...
void compress_image_multilevel(Image* img, const char* output_prefix, int levels) {
    // HOTSPOT 8: Multi-level compression - combines multiple operations
    ...
    for (int level = 1; level <= levels; level++) {
        ...
        // Step 1: Apply Gaussian pre-filter to prevent aliasing
        double prefilter_start = get_time();
        float sigma = 0.8f * level; // Increase sigma with level
        ...
        snprintf(filename, sizeof(filename), "sequential output images/%s_compressed_level_%d.png", output_prefix, level);
    ...
    snprintf(filename, sizeof(filename), "sequential output images/%s_final_compressed.png", output_prefix);
